...
These workflows are fixed to the current five volume directories. Future
repositories `lra-volume-vi`, `lra-volume-vii`, and `lra-volume-viii` will need
matching workflows.
...
It does not currently watch Volume I, IV, V, or any future Volume VI-VIII.
This may be intentional extraction scoping, but the target architecture should
make that explicit.
...
6. Decide whether `trigger-knowledge-rebuild.yml` should watch all volumes or
   only extraction-active volumes.
...
- The knowledge explorer currently watches only Volume II and Volume III.
