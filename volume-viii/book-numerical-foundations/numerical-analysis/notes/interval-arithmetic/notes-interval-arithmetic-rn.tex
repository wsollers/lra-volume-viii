...
\subsection{\texorpdfstring{Interval Arithmetic in $\mathbb{R}^n$}{Interval Arithmetic in R n}}
\label{sec:interval-arithmetic-rn}

\begin{definitionbox}{Definition (Box / Product Interval)}
\begin{definition}[Box / Product Interval]\label{def:product-interval-box}
...
\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:product-interval-box]{Box / Product Interval}
...
Many applications also require quick bounds on a norm over a box.
For example, if $0\le a\le b$ coordinatewise, then for every $x\in[a,b]$
